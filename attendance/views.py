...
from students.models import Student, Badge, StudentBadge
...
from dashboard.models import VolunteerNote, AdminLog, StudentViolation, EventSettings
...
def _register_attendance_for_session(student, session, status_value):
    ...
    if session:
        ...
        if created:
            ...
            if status_value == Attendance.Status.PRESENT:
                ...
                # التحقق من إعدادات الفعالية للسماح بنقاط المجموعات
                if student.group and EventSettings.get_solo().allow_group_points:
    ...
    # Check and award badges after attendance/points change
...
def scan_qr(request):
    ...
    if request.method == 'POST':
        if not event.allow_group_points and not request.user.is_admin():
        ...
        # منع تسجيل الحضور خارج وقت الورشة للمشرفين والمتطوعين
        ...
        if not session and not is_admin:
            return JsonResponse(
                {
                    'success': False,
                    'message': 'لا توجد ورشة نشطة حالياً لهذا الطالب لمسح الحضور.',
                }
            )

        workshop_title, is_event_already_checked_in, points_awarded, is_session_already_registered, new_badges = _register_attendance_for_session(student, session, status_value)
...
def scan_session_qr(request, session_id):
    ...
    checked_in_not_present_count = Student.objects.filter(
        group=session.group,
        checked_in=True
    ).exclude(id__in=already_present_ids).count()
...
def mark_all_group_present(request, session_id):
    ...
    students_to_mark = Student.objects.filter(
        group=session.group,
        checked_in=True
    ).exclude(id__in=already_present_ids)
    ...
    for student in students_to_mark:
        ...
        # إضافة النقاط إذا كان جديداً أو كانت حالته السابقة غير PRESENT
        # ملاحظة: update_or_create يعيد (obj, created)
        # نحن نعتمد على المنطق في _register_attendance_for_session للتماسك
        # لكن هنا سنقوم بذلك يدوياً للأداء في حال عدد كبير من الطلاب
