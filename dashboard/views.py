...
def admin_schedule(request):
    ...
    # تحويل المواعيد من نص إلى كائنات time للمقارنة
    ...
    for value, label in WorkshopSession.PERIOD_CHOICES:
        ...
        def parse_time(t_str):
            ...
            # تصحيح للساعات المسائية (1, 2 ظهرًا)
            if h < 8: h += 12
            return time(h, m)
    ...
    # بناء مصفوفة الجدول (Matrix)
    schedule_matrix = []
    for period in period_definitions:
        row = {'period': period, 'group_sessions': []}
        for group in groups:
            # البحث عن أي جلسة تتداخل مع هذا الوقت لهذه المجموعة
            session = None
            for s in sessions:
            ...
            # تحديد نوع النشاط التلقائي إذا لم توجد جلسة
            activity_label = None
            if not session:
                g_code = group.code.upper()
                p_val = period['value']
                
                # منطق الألعاب حسب المجموعات من نص المستخدم
                if g_code in ['A', 'F', 'I'] and p_val in ['9:55-10:05', '10:05-10:40']:
                    activity_label = 'نشاط ترفيهي (ألعاب)'
                elif g_code in ['B', 'C', 'G'] and p_val in ['11:00-11:10', '11:10-11:45']:
                    activity_label = 'نشاط ترفيهي (ألعاب)'
                elif g_code in ['D', 'E', 'H'] and p_val in ['12:05-12:50']:
                    activity_label = 'نشاط ترفيهي (ألعاب)'
                elif 'انتقال' in period['label']:
                    activity_label = 'فترة انتقال'
                elif 'تسجيل' in period['label']:
                    activity_label = 'تسجيل الدخول'
                elif 'الخاتمة' in period['label']:
                    activity_label = 'الختام والشهادات'

            row['group_sessions'].append({
                'group': group,
                'session': session,
...
def admin_schedule_random_assign(request):
    ...
    # الفترات التي يتم فيها توزيع ورش فعلياً حسب الجدول الجديد
    workshop_periods = [
        '9:00-9:55',   # الورشة الأولى (الكل)
        '10:05-10:40', # الورشة الثانية (مبكر)
        '10:50-11:00', # استمرار ورشة 2 (متأخر)
        '11:10-11:45', # الورشة الثالثة (مبكر)
        '11:55-12:05', # الورشة الثالثة (متأخر)
        '12:05-12:50', # استمرار ورشة 3
        '1:00-1:55'    # الورشة الرابعة (الكل)
    ]
    
    period_time_map = {
        '9:00-9:55': (time(9, 0), time(9, 55)),
        '10:05-10:40': (time(10, 5), time(10, 40)),
        '10:40-10:50': (time(10, 40), time(10, 50)),
